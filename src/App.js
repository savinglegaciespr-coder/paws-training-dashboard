...
const COLORS = {
  ...
  card:       "#2D2D2D",
  cardBorder: "#3D3D3D",
  accent:     "#6BBF3E",   // verde lima — acento principal
  accentSoft: "#6BBF3E20",
  green:      "#39D353",   // verde brillante — éxito, Agresivo, 1099 (distinto de accent)
  greenSoft:  "#39D35320",
  red:        "#F85149",
  redSoft:    "#F8514920",
  blue:       "#A8D8A8",   // verde pálido — secundario, Conservador
  blueSoft:   "#A8D8A820",
  text:       "#E8E8E8",
  textMuted:  "#9A9A9A",
  textDim:    "#5A5A5A",
...
const DEFAULTS = {
  ...
  renta: 1200, electricidad: 300, agua: 80, internet: 80, seguro: 200,
  suministros: 200, marketing: 100, otrosGastos: 0,
  empleados: [
...
export default function App() {
  ...
  // ── Cálculos con fallbacks seguros (SIEMPRE, antes de cualquier return) ────
  const empleados      = Array.isArray(data.empleados) ? data.empleados : [];
  const ingresoEntrenamiento = data.ingresoEntrenamiento || 0;
  const ingresoGuarderia     = data.ingresoGuarderia     || 0;
  const ingresoGrooming      = data.ingresoGrooming      || 0;
  const ingresoOtros         = data.ingresoOtros         || 0;
  const renta          = data.renta        || 0;
  const electricidad   = data.electricidad || 0;
  const agua           = data.agua         || 0;
  const internet       = data.internet     || 0;
  const seguro         = data.seguro       || 0;
  const suministros    = data.suministros  || 0;
  const marketing      = data.marketing    || 0;
  const otrosGastos    = data.otrosGastos  || 0;
  const capacidadActual    = data.capacidadActual    || 0;
  const ocupacionActual    = data.ocupacionActual    || 0;
  const precioPromedio     = data.precioPromedio     || 0;
  const nuevaContratacion  = data.nuevaContratacion  || 0;

  const totalIngresos   = ingresoEntrenamiento + ingresoGuarderia + ingresoGrooming + ingresoOtros;
  const totalNomina     = empleados.reduce((s, e) => s + (e.pago || 0), 0);
  const costoNominaTotal = empleados.reduce((s, e) => s + (e.pago || 0) + (e.tipo === "W-2" ? (e.pago || 0) * 0.15 : 0), 0);
  ...
  // ── useMemo SIEMPRE antes del return condicional ───────────────────────────
  ...
  const addEmpleado    = () => update("empleados", [...empleados, { nombre: "Nuevo Empleado", pago: 1200, horasSemana: 40, tipo: "1099" }]);
  const removeEmpleado = (i) => update("empleados", empleados.filter((_, idx) => idx !== i));
          ...
          <div>
            <Section title="Empleados / Contratistas" icon="👤">
              <div style={{ overflowX: "auto" }}>
                <div style={{ minWidth: 480 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr auto", gap: 8, marginBottom: 8, padding: "4px 12px" }}>
                    {["Nombre / Rol", "Pago/mes", "Hrs/Sem", "Tipo", ""].map(h => (
                      <span key={h} style={{ fontSize: 10, color: COLORS.textDim, textTransform: "uppercase" }}>{h}</span>
                    ))}
                  </div>
                  {empleados.map((emp, i) => (
              ...
              <p style={{ fontSize: 10, color: COLORS.textDim, marginBottom: 8, marginTop: 4 }}>
                💡 Toca <strong style={{ color: COLORS.green }}>1099</strong> o <strong style={{ color: COLORS.accent }}>W-2</strong> para cambiar. Los 1099 no tienen cargas patronales.
              </p>
            ...
            <Section title="Costo Laboral Total" icon="💼">
              <div style={g3}>
                {empleados.map((emp, i) => {
                  const cargas = emp.tipo === "W-2" ? (emp.pago || 0) * 0.15 : 0;
                  const costo  = (emp.pago || 0) + cargas;
                  return (
